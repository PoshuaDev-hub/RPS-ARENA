...
// Iconos Font Awesome integrados directamente como SVG
const icons = {
...
function showResultView() {
    ...
    // Inyectamos el SVG y aplicamos el filtro de neón
    iconP1.innerHTML = icons[p1Choice];
    iconP1.style.color = 'var(--p1)';
    iconP1.style.filter = 'drop-shadow(0 0 12px var(--p1))';
    
    iconP2.innerHTML = icons[p2Choice];
    iconP2.style.color = 'var(--p2)';
    iconP2.style.filter = 'drop-shadow(0 0 12px var(--p2))';
...
function determinarGanador() {
    ...
    if (p1Choice === p2Choice) {
        status.innerText = "EMPATE TÉCNICO EN EL COSMOS";
...
function resetGame() {
    ...
    badge.className = "p1-theme";
}

// --- MOTOR VISUAL (Estrellas y Fugaces) ---
function initStars() {
...
window.onload = () => {
    // Inyectamos los iconos iniciales en la arena radial
    document.getElementById('svg-rock').innerHTML = icons.rock;
    document.getElementById('svg-paper').innerHTML = icons.paper;
    document.getElementById('svg-scissors').innerHTML = icons.scissors;
    initStars();
